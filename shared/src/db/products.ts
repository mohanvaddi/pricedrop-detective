...
export async function findAllActiveProducts(): Promise<EnrichedProduct[]> {
  try {
    ...
    const rows = await db.execute<Row>(sql`
      ...
      GROUP BY p.id, pm.initial_price, pm.current_price, pm.all_time_low, pm.available, wu.display_name
    `);
...
export async function insertProduct(
  ...
  try {
    await db.insert(products).values({ id: hash, url, website, title, thumbnailUrl, category });
  } catch (error) {
...
/**
 * Products that are due for a re-scrape: never scraped yet, or the effective
 * scrapeInterval has elapsed since lastScrapedAt. Used by the scrapers worker.
 */
export async function findDueProducts(): Promise<Product[]> {
  try {
    const rows = await db.execute<Product>(sql`
      SELECT p.*
      FROM products p
      JOIN subscriptions s ON s.product_id = p.id
      LEFT JOIN product_metrics pm ON pm.product_id = p.id
      WHERE pm.last_scraped_at IS NULL
         OR pm.last_scraped_at <= now() - (p.scrape_interval || ' seconds')::interval
      GROUP BY p.id
    `);
    return rows.rows as unknown as Product[];
  } catch (error) {
    throw new CustomError('Unable to fetch due products', 'ProductsError', { error });
  }
}

/** Persist a recomputed scrape interval + priority tier for a product. */
export async function updateScrapeSchedule(productId: string, scrapeInterval: number, priority: string): Promise<void> {
  try {
    await db.update(products).set({ scrapeInterval, priority }).where(eq(products.id, productId));
  } catch {
    // Non-fatal
  }
}

/** Persist a resolved category / product type for a product. */
export async function updateProductCategory(productId: string, category: string | null, productType: string | null): Promise<void> {
  try {
    await db.update(products).set({ category, productType }).where(eq(products.id, productId));
  } catch (error) {
    throw new CustomError('Unable to update product category', 'ProductUpdateError', { error });
  }
}

/** Products that have not been categorized yet — used by the recategorize script. */
export async function findUncategorizedProducts(): Promise<Product[]> {
  try {
    return await db.select().from(products).where(sql`${products.category} IS NULL`);
  } catch (error) {
    throw new CustomError('Unable to fetch uncategorized products', 'ProductsError', { error });
  }
}

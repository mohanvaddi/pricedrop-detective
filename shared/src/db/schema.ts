import { pgTable, text, integer, boolean, timestamp, bigint, uuid, index, unique, jsonb } from 'drizzle-orm/pg-core';
...
// ---------------------------------------------------------------------------
// 11. Notification queue — durable outbox filled by the scrapers worker and
//     drained by the server's notification poller.
// ---------------------------------------------------------------------------
export const notificationQueue = pgTable('notification_queue', {
  ...
  index('notification_queue_status_idx').on(t.status, t.createdAt),
]);

// ---------------------------------------------------------------------------
// Scraper sessions — auto-generated browser sessions (cookies + UA) reused by
// the cheap curl fast-path for session/cookie-based scraping (e.g. AJIO/Akamai).
// One row per platform; regenerated by Camoufox when the session expires.
// ---------------------------------------------------------------------------
export const scraperSessions = pgTable('scraper_sessions', {
  platform: text('platform').primaryKey(),
  cookie: text('cookie').notNull(), // curl `-b` cookie string
  userAgent: text('user_agent').notNull(),
  headers: jsonb('headers').$type<Record<string, string>>(), // extra request headers to replay
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
...
export type ScraperSession = typeof scraperSessions.$inferSelect;
export type NewScraperSession = typeof scraperSessions.$inferInsert;
